...
local function watchForRequests(colony)
    ...
    local reqLogs = fs.open("CentralPrograms/Logs/requestsLogs.log", "w")
    ...
    do
        if string.sub(requests[i].target, 1, 7) == "Builder" then
            ...
            table.insert(toGet,
                {
                    id = requests[i].items[1].name,
                    count = requests[i].count,
                    nbt = requests[i].items[1].nbt,
                    displayName = requests[i].items[1].displayName,
                    available = 0
                })
