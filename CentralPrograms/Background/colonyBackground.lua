...
local requestWatcher = require("Helpers.requestWatcher")
local helpers = require("Helpers.mainHelpers")
local function startCheck(interval, meBridge, colony,
                          logInterval)
    print("Job Started.")
    local logs = fs.open("CentralPrograms/Logs/colonyLogs.log", "w")
    ...
    while true do
        if logInterval == nbLoopSinceLog then
            ...
            logs = fs.open("CentralPrograms/Logs/colonyLogs.log", "w")
        end

        local uncraftableItems = {}     --names only, represent the list of items
        local uncraftableItemsArgs = {} --what is passed to the Front
        ...
        if table.getn(requests) == 0 then
            ...
            for i = 1, table.getn(requests), 1 do
                ...
                if ae2Items[itemName] == nil or ae2Items[itemName].amount < requests[i].count then -- item not found or not enougth
                    ...
                    if ae2Items[itemName] ~= nil then
                        requests[i].available = ae2Items[itemName].amount
                    end
        ...
        --update the list
        MineColonyHelpers.RefreshUncraftableList(uncraftableItemsArgs)
